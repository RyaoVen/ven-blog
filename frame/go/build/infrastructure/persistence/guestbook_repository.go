...
func (r *GuestbookRepository) ClaimAIReview(id int64) (bool, error) {
	res, err := r.db.Exec("UPDATE guestbook SET ai_reviewed_at = NOW() WHERE id = ? AND status = ? AND ai_reviewed_at IS NULL", id, guestbook.StatusPending)
	if err != nil {
		return false, fmt.Errorf("claim ai review of guestbook entry %d: %w", id, err)
	}
...
func (r *GuestbookRepository) UnclaimAIReview(id int64) error {
	if _, err := r.db.Exec("UPDATE guestbook SET ai_reviewed_at = NULL WHERE id = ? AND status = ?", id, guestbook.StatusPending); err != nil {
		return fmt.Errorf("unclaim ai review of guestbook entry %d: %w", id, err)
	}
	return nil
}
