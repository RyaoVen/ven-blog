...
func DetectWithRoot(root string) DetectResult {
	...
	r.Port3000 = PortOpen("127.0.0.1:3000", 500*time.Millisecond)
	r.Port8080 = PortOpen("127.0.0.1:8080", 500*time.Millisecond)
	if root != "" {
...
func Check(root string, w io.Writer) error {
	...
	port3000 := "空闲（Node worker 就绪位）"
	if r.Port3000 {
		port3000 = "占用（疑似已在运行）"
	}
	fmt.Fprintf(w, "[端口]    :3000 %-34s %s\n", port3000, mark(!r.Port3000))
	port8080 := "空闲（Go 网关就绪位）"
	if r.Port8080 {
		port8080 = "占用（疑似已在运行）"
	}
	fmt.Fprintf(w, "[端口]    :8080 %-34s %s\n", port8080, mark(!r.Port8080))
